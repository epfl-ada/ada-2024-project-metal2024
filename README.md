...
`Ngrams` are contiguous `N` words repeated in the text preprocessed summaries, those with the most occurences are shown first. In their `tf_idf` variant, their are ranked according to their `tf` times `idf` [score](https://en.wikipedia.org/wiki/Tf%E2%80%93idf). `Named_entities` are classified words are characters in different categories as presented in the corresponding `.csvs`. These are not computed by us and are extracted from the `coreNLP` preprocessing text summaries linked alongside the dataset. As said before, we then group these by `year`, `decade`, and `period`s. The `periods` are defined by us in `results.ipynb` as well as in `utils.py`.